...
async function nadraCheck(cnic: string, name: string) {
...
export async function POST(req: NextRequest) {
  ...
  if (!nadraVerified) {
    return NextResponse.json({ error: 'NADRA verification failed.' }, { status: 400 });
  }
  ...
  console.log(`Verify email: http://localhost:3000/verify-email?token=${token}`);

  return NextResponse.json({ message: 'Registered. Please verify your email.' });
} 